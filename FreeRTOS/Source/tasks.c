...
/*-----------------------------------------------------------*/
	...
	PRIVILEGED_DATA static char pcStatusString[ 50 ];
		...
		do
		{
			...
			sprintf( pcStatusString, ( char * ) "\t\t%c\t%u\t%u\t%u\r\n", cStatus, ( unsigned int ) pxNextTCB->uxPriority, usStackRemaining, ( unsigned int ) pxNextTCB->uxTCBNumber );
		...
		do
		{
			...
			if( ulTotalRunTime > 0UL )
			{
				...
				if( pxNextTCB->ulRunTimeCounter == 0UL )
				{
					...
					sprintf( pcStatsString, ( char * ) "\t\t0\t\t0%%\r\n" );
				}
				else
				{
					...
					if( ulStatsAsPercentage > 0UL )
					{
						...
						{
							sprintf( pcStatsString, ( char * ) "\t\t%lu\t\t%lu%%\r\n", pxNextTCB->ulRunTimeCounter, ulStatsAsPercentage );							
						}
						#else
						{
							/* sizeof( int ) == sizeof( long ) so a smaller
							printf() library can be used. */
							sprintf( pcStatsString, ( char * ) "\t\t%u\t\t%u%%\r\n", ( unsigned int ) pxNextTCB->ulRunTimeCounter, ( unsigned int ) ulStatsAsPercentage );
						}
						#endif
					}
					else
