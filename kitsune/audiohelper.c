...
uint8_t InitFile(Filedata_t * pfiledata) {
	...
	uint8_t ret = 1;
	/* open file */
	res = hello_open(&pfiledata->file_obj, pfiledata->file_name, FA_WRITE|FA_OPEN_ALWAYS);
	...
	if(res != FR_OK && res != FR_EXIST){
		...
		hello_stat(pfiledata->file_name, &file_info);

		if(file_info.fsize != 0 ){
			res = hello_lseek(&pfiledata->file_obj, file_info.fsize);
		}
	}
...
uint8_t WriteToFile(Filedata_t * pfiledata,const WORD bytes_to_write,const uint8_t * const_ptr_samples_bytes) {
	...
	do {
		res = hello_write(&pfiledata->file_obj, const_ptr_samples_bytes +  bytes_written , bytes_to_write-bytes_written, &bytes );
...
void CloseFile(Filedata_t * pfiledata) {
	hello_close(&pfiledata->file_obj)	;
