...
int hlo_filter_lossless_encoder(hlo_stream_t * input, hlo_stream_t * output, void * ctx, hlo_stream_signal signal){
	...
	const int8_t channels = 1;
	const int32_t estimated_frames = 800; //6 seconds
	...
	ret &= 0<hlo_stream_transfer_all(INTO_STREAM, output, (uint8_t*)&magic_number,sizeof(magic_number), 4);
	ret &= 0<hlo_stream_transfer_all(INTO_STREAM, output, (uint8_t*)&sample_rate,sizeof(int32_t), 4);
	ret &= 0<hlo_stream_transfer_all(INTO_STREAM, output, (uint8_t*)&bps,sizeof(int16_t), 4);
	ret &= 0<hlo_stream_transfer_all(INTO_STREAM, output, (uint8_t*)&channels,sizeof(int8_t), 4);
	ret &= 0<hlo_stream_transfer_all(INTO_STREAM, output, (uint8_t*)&estimated_frames,sizeof(int32_t), 4);

	//Define read size
	int32_t read_size = channels * BLOCK_SIZE;
	int16_t * buffer = short_samples;

	while(ret >= 0){
		ret = hlo_stream_transfer_all(FROM_STREAM, input, (uint8_t*)short_samples, read_size*bps/8, 4);
		if ( ret < 0 ){
			break;
		}
		ret /= bps/8;

		samples_per_channel = ret/channels;
		...
		for(i = 0; i < channels; i++)
		{
			...
			ret &= 0<hlo_stream_transfer_all(INTO_STREAM, output, (uint8_t*)&rice_param_ref,sizeof(uint8_t), 4);
			ret &= 0<hlo_stream_transfer_all(INTO_STREAM, output, (uint8_t*)&req_int_ref,sizeof(uint16_t), 4);
			ret &= 0<hlo_stream_transfer_all(INTO_STREAM, output, (uint8_t*)&opt_lpc_order,sizeof(uint8_t), 4);
			ret &= 0<hlo_stream_transfer_all(INTO_STREAM, output, (uint8_t*)encoded_ref,req_int_ref*sizeof(uint32_t), 4);

			//Write rice_params,bytes,encoded residues to output
			ret &= 0<hlo_stream_transfer_all(INTO_STREAM, output, (uint8_t*)&rice_param_residue,sizeof(uint8_t), 4);
			ret &= 0<hlo_stream_transfer_all(INTO_STREAM, output, (uint8_t*)&req_int_residues,sizeof(uint16_t), 4);
			ret &= 0<hlo_stream_transfer_all(INTO_STREAM, output, (uint8_t*)&encoded_residues,req_int_residues*sizeof(uint32_t), 4);
