...
// 
// This is part of revision 8028 of the Stellaris Firmware Development Package.
//
// Modifications:
//		5/18/2012: By Ken Pettit
//                    Fuel7, Inc.
//
//                 -  Added support for command line history 
//                        Uses WANT_CMDLINE_HISTORY macro for inclusion
//
//                 -  Added support for signaling FreeRTOS task from Rx ISR.
//                        Uses WANT_FREERTOS_SUPPORT macro for inclusion.
//
//                 -  Set the RX interrupt priority when running under FreeRTOS
//
//*****************************************************************************
...
#ifdef WANT_FREERTOS_SUPPORT
// semaphore for signalling Rx lines
xSemaphoreHandle	g_xRxLineSemaphore;
#endif /* WANT_FREERTOS_SUPPORT */
...
UARTStdioInitExpClk(unsigned long ulPortNum, unsigned long ulBaud)
{
    ...
    MAP_IntPrioritySet(g_ulUARTInt[ulPortNum], configMAX_SYSCALL_INTERRUPT_PRIORITY);
	vSemaphoreCreateBinary(g_xRxLineSemaphore);
#endif /* WANT_FREERTOS_SUPPORT */
        ...
        while(MAP_UARTCharsAvail(g_ulBase))
        {
            ...
            if(!g_bDisableEcho)
            {
						...
						if ((cChar == 'A') || (cChar == 'B'))
						{
							...
							for (x = 0; g_pcUARTRxHistoryBuffer[copyFrom] != '\0'; x++)
							{
								g_pcUARTRxBuffer[g_ulUARTRxWriteIndex] = 
									g_pcUARTRxHistoryBuffer[copyFrom++];
								UARTwrite((const char *) &g_pcUARTRxBuffer[g_ulUARTRxWriteIndex], 1);
				                ADVANCE_RX_BUFFER_INDEX(g_ulUARTRxWriteIndex);
								if (copyFrom >= UART_RX_HISTORY_BUF_SIZE)
									copyFrom = 0;
							}
							g_pcUARTRxBuffer[g_ulUARTRxWriteIndex] = '\0';	
                ...
                if((cChar == '\r') || (cChar == '\n') || (cChar == 0x1b))
                {
                    ...
                    //
                    // Signal the global FreeRTOS semaphore for an Rx line
                    //
                    #ifdef WANT_FREERTOS_SUPPORT
