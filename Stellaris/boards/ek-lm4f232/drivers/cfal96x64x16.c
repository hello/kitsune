...
//    0xAE,         // display off
    ...
    0xA0, 0x60,//00 // remap and data format - use 16-bit color mode
    0xBB, 0x1F,     // Vpa
    0xBC, 0x1F,     // Vpb
    0xBD, 0x1F,     // Vpc
...
//*****************************************************************************
static void
CFAL96x64x16WriteData(const unsigned short *pcData, unsigned long ulCount)
{
    ...
    {
    	color = *pcData;
        ROM_SSIDataPut(DISPLAY_SSI_BASE, color>>8);
        ROM_SSIDataPut(DISPLAY_SSI_BASE, color&0xff);
        pcData++;
    }
}
                      ...
                      unsigned long ulValue)
{
    ...
    //
    CFAL96x64x16WriteData((unsigned short *)&ulValue, 1);
}
                              ...
                              const unsigned char *pucPalette)
{
    ...
    switch(lBPP)
    {
        ...
        case 1:
        {
            ...
            while(lCount)
            {
                ...
                for(; (lX0 < 8) && lCount; lX0++, lCount--)
                {
                    ...
                    //
                    unsigned short ucBPP = ((unsigned long *)pucPalette)
                                            [(ulByte >> (7 - lX0)) & 1];
        ...
        //
        // The pixel data is in 656 bit per pixel format.
        //
        case 656:
            //
            // Loop while there are more pixels to draw.
            ...
            while(lCount--)
            {
                ...
                //
                ucColor = *pucData++;
                ucColor |= ((unsigned short) *pucData++) << 8;
...
CFAL96x64x16LineDrawH(void *pvDisplayData, long lX1, long lX2, long lY,
                      unsigned long ulValue)
{
    unsigned short ucLineBuf[16];
    unsigned char* pCmdBuf = (unsigned char *) ucLineBuf;
    unsigned int uIdx;
    ...
    //
    pCmdBuf[0] = 0x15;
    pCmdBuf[1] = lX1 < lX2 ? lX1 : lX2;
    pCmdBuf[2] = 95;
    pCmdBuf[3] = 0x75;
    pCmdBuf[4] = lY;
    pCmdBuf[5] = 63;
    CFAL96x64x16WriteCommand(pCmdBuf, 6);

    //
    // Use buffer of pixels to draw line, so multiple bytes can be sent at
    // one time.  Fill the buffer with the line color.
    //
    for(uIdx = 0; uIdx < sizeof(ucLineBuf)/sizeof(unsigned short); uIdx++)
    {
                      ...
                      unsigned long ulValue)
{
    ...
    pCmdBuf[2] = 95;
    pCmdBuf[3] = 0x75;
    pCmdBuf[4] = lY1 < lY2 ? lY1 : lY2;
    pCmdBuf[5] = 63;
    pCmdBuf[6] = 0xA0;
    pCmdBuf[7] = 0x21;
    CFAL96x64x16WriteCommand(pCmdBuf, 8);
    ...
    pCmdBuf[0] = 0xA0;
    pCmdBuf[1] = 0x60;
    CFAL96x64x16WriteCommand(pCmdBuf, 2);
}
