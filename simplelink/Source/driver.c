...
void             _SlDrvShiftDWord(UINT8 *pBuf);
void             _SlDrvDriverCBInit(void);
void             _SlAsyncEventGenericHandler(void);
int				 _SlDrvWaitForPoolObj(UINT32 ActionID, UINT8 SocketID);
void			 _SlDrvReleasePoolObj(UINT8 pObj);
void			 _SlDrvObjInit(void);
void			 _SlDrvObjDeInit(void);
void			 _SlRemoveFromList(UINT8* ListIndex, UINT8 ItemIndex);
_SlReturnVal_t	 _SlFindAndSetActiveObj(_SlOpcode_t  Opcode, UINT8 Sd);
...
#define N2H_SYNC_PATTERN_SEQ_NUM_BITS            ((UINT32)0x00000003) // Bits 0..1    - use the 2 LBS for seq num
#define N2H_SYNC_PATTERN_SEQ_NUM_EXISTS          ((UINT32)0x00000004) // Bit  2       - sign that sequence number exists in the sync pattern
#define N2H_SYNC_PATTERN_MASK                    ((UINT32)0xFFFFFFF8) // Bits 3..31   - constant SYNC PATTERN
#define N2H_SYNC_SPI_BUGS_MASK                   ((UINT32)0x7FFF7F7F) // Bits 7,15,31 - ignore the SPI (8,16,32 bites bus) error bits 
#define BUF_SYNC_SPIM(pBuf)                      ((*(UINT32 *)(pBuf)) & N2H_SYNC_SPI_BUGS_MASK)
...
#define N2H_SYNC_PATTERN_MATCH(pBuf, TxSeqNum) \
            ( \
                 (  (*((UINT32 *)pBuf) & N2H_SYNC_PATTERN_SEQ_NUM_EXISTS) && ( MATCH_WITH_SEQ_NUM(pBuf, TxSeqNum) ) )	|| \
                 ( !(*((UINT32 *)pBuf) & N2H_SYNC_PATTERN_SEQ_NUM_EXISTS) && ( MATCH_WOUT_SEQ_NUM(pBuf          ) ) )	   \
            )
...
void _SlDrvRxIrqHandler(void *pValue)
{
    ...
    {
        OSI_RET_OK_CHECK( sl_SyncObjSignalFromISR(&g_pCB->CmdSyncObj) );
    }
    else
    {
...
//*****************************************************************************
//  _SlDrvCmdSend 
//  Send SL command without waiting for command response - This function is unprotected and the caller should make sure global lock is active
//*****************************************************************************
    ...
    _SlCmdExt_t         *pCmdExt)
{
	...
	pObjIdx = _SlDrvWaitForPoolObj(RECV_ID, Sd & BSD_SOCKET_ID_MASK);
	...
	pArgsData.pArgs =  (UINT8 *)pTxRxDescBuff;
	g_pCB->ObjPool[pObjIdx].pRespArgs =  (UINT8 *)&pArgsData;


    // Do Flow Control check/update for DataRead operation
...
_SlReturnVal_t _SlDrvMsgWrite(void)
{
	...
	{
		g_pCB->TempProtocolHeader.Len = g_pCB->TempProtocolHeader.Len + 4;
	}
...
#endif 
    // Sync
    NWP_IF_WRITE_CHECK(g_pCB->FD, (unsigned char *)&g_H2NSyncPattern.Short, SYNC_PATTERN_LEN);

    // Header
    NWP_IF_WRITE_CHECK(g_pCB->FD, (unsigned char *)&g_pCB->TempProtocolHeader, _SL_CMD_HDR_SIZE);
    ...
    if (g_pCB->RelayFlagsViaRxPayload == TRUE)
    {
        ...
        TEMP = (UINT32)g_pCB->FunctionParams.pCmdExt->pRxPayload;
        NWP_IF_WRITE_CHECK(g_pCB->FD, (unsigned char*)&TEMP,_SL_PROTOCOL_ALIGN_SIZE(4));
    }
    ...
    if (g_pCB->FunctionParams.pCmdExt && g_pCB->FunctionParams.pCmdExt->TxPayloadLen > 0)
    {
        ...
        // Otherwise the alignning of arguments will create a gap between argumants and payload.
...
_SlReturnVal_t _SlDrvMsgRead(void)
{
    // aligment for small memory models
    union
    {      
      UINT8                TempBuf[_SL_RESP_HDR_SIZE];
      UINT32               DummyBuf[2];
    } uBuf;
    UINT8                TailBuffer[4];
    UINT16               LengthToCopy;
    UINT16               AlignedLengthRecv;
    UINT8                AlignSize;

    VERIFY_RET_OK(_SlDrvRxHdrRead((UINT8*)(uBuf.TempBuf), &AlignSize));
    ...
    switch(g_pCB->FunctionParams.AsyncExt.RxMsgClass)
    {
        case ASYNC_EVT_CLASS:
            ...
            VERIFY_PROTOCOL(_SL_PROTOCOL_ALIGN_SIZE(RSP_PAYLOAD_LEN(uBuf.TempBuf))<= SL_ASYNC_MAX_PAYLOAD_LEN);

            if (RSP_PAYLOAD_LEN(uBuf.TempBuf) > 0)
            {
                NWP_IF_READ_CHECK(g_pCB->FD,
                                   g_pCB->FunctionParams.AsyncExt.pAsyncBuf + _SL_RESP_HDR_SIZE,
                                   _SL_PROTOCOL_ALIGN_SIZE(RSP_PAYLOAD_LEN(uBuf.TempBuf)));
        ...
        case RECV_RESP_CLASS:
            {
                UINT8   ExpArgSize; // Expected size of Recv/Recvfrom arguments
        ...
        case CMD_RESP_CLASS:
            ...
            if((NULL != g_pCB->FunctionParams.pCmdExt) && (0 != g_pCB->FunctionParams.pCmdExt->RxPayloadLen))
            {
                ...
                INT16    ActDataSize = RSP_PAYLOAD_LEN(uBuf.TempBuf) - g_pCB->FunctionParams.pCmdCtrl->RxDescLen;
                ...
                else
                {
                    ...
                    g_pCB->FunctionParams.pCmdExt->RxPayloadLen = ActDataSize;
                }
            }
            break;
         ...
         case SL_OPCODE_NETAPP_IPACQUIRED:
			{
			 ...
			 netAppEvent.Event = SL_NETAPP_IPV4_ACQUIRED;
         ...
         case SL_OPCODE_NETAPP_IPACQUIRED_V6:
           {
			...
			netAppEvent.Event =  SL_NETAPP_IPV6_ACQUIRED;
            ...
            case SL_OPCODE_NETAPP_IP_LEASED:
			{
			 ...
			 netAppEvent.Event = SL_NETAPP_IP_LEASED;
            ...
            case SL_OPCODE_NETAPP_IP_RELEASED:
			{
			 ...
			 netAppEvent.Event = SL_NETAPP_IP_RELEASED;
         ...
         case SL_OPCODE_SOCKET_TXFAILEDASYNCRESPONSE:
			{
				...
				sockAppEvent.Event =  SL_NETAPP_SOCKET_TX_FAILED;
          ...
          case SL_OPCODE_WLAN_SMART_CONFIG_START_ASYNC_RESPONSE:
          {
                ...
                wlanEvent.Event = SL_WLAN_SMART_CONFIG_START_EVENT;
          ...
          {
                slStaConnectedAsyncResponse_t* pResp = (slStaConnectedAsyncResponse_t*)_SL_RESP_ARGS_START(g_pCB->FunctionParams.AsyncExt.pAsyncBuf);

                wlanEvent.Event = SL_WLAN_STA_CONNECTED_EVENT;
                memcpy(wlanEvent.EventData.staConnected.mac,pResp->mac, 6);
          ...
          case SL_OPCODE_WLAN_STA_DISCONNECTED:
          {
                ...
                memcpy(wlanEvent.EventData.staDisconnected.mac,pResp->mac, 6);
...
_SlReturnVal_t _SlDrvMsgReadCmdCtx(void)
{
    ...
    // Async messages in Commands contex, because ssiDma_IsrHandleSignalFromSlave
	...
	if (0 == (SL_OPCODE_SYNC & Opcode))
	{
		...
		else
		{
			...
			else if (SL_OPCODE_SILO_WLAN == (Opcode & SL_OPCODE_SILO_MASK))
			{
				 ...
				 g_pCB->FunctionParams.AsyncExt.AsyncEvtHandler = sl_WlanEvtHdlr;
#endif
			}
			else if (SL_OPCODE_SILO_SOCKET == (Opcode & SL_OPCODE_SILO_MASK))
			{
				...
				g_pCB->FunctionParams.AsyncExt.AsyncEvtHandler = sl_SockEvtHdlr;
#endif
			}
			else if (SL_OPCODE_SILO_NETAPP == (Opcode & SL_OPCODE_SILO_MASK))
			{
				...
				{
					g_pCB->FunctionParams.AsyncExt.AsyncEvtHandler = sl_NetAppEvtHdlr;
				}
#endif
			}
...
_SlReturnVal_t   _SlDrvRxHdrRead(UINT8 *pBuf, UINT8 *pAlignSize)
{
    ...
    // 1. Write CNYS pattern to NWP
    NWP_IF_WRITE_CHECK(g_pCB->FD, (unsigned char *)&g_H2NCnysPattern.Short, SYNC_PATTERN_LEN);
    ...
    while ( ! N2H_SYNC_PATTERN_MATCH(pBuf, g_pCB->TxSeqNum) )
    {
        ...
        if(0 == (SyncCnt % (UINT32)SYNC_PATTERN_LEN))
        {
    ...
    if(SyncCnt > 0)
    {
        *(UINT32 *)&pBuf[0] = *(UINT32 *)&pBuf[4];
        NWP_IF_READ_CHECK(g_pCB->FD, &pBuf[SYNC_PATTERN_LEN - SyncCnt], SyncCnt);
    }
    else
    {
    	NWP_IF_READ_CHECK(g_pCB->FD, &pBuf[0], 4);
...
int _SlDrvWaitForPoolObj(UINT32 ActionID, UINT8 SocketID)
{
	UINT8 CurrObjIndex = MAX_CONCURRENT_ACTIONS;
	...
	g_pCB->ObjPool[CurrObjIndex].ActionID = ActionID;
	...
	//In case SocketID is set to SL_MAX_SOCKETS, the socket is not relevent to the action. In that case ActionID bit will be on
	if ( ( (SL_MAX_SOCKETS > SocketID) && (g_pCB->ActiveActionsBitmap & (1<<SocketID)) ) || ( (g_pCB->ActiveActionsBitmap & (1<<ActionID)) && (SL_MAX_SOCKETS == SocketID) ) )
	{
...
//
//*****************************************************************************
void _SlDrvReleasePoolObj(UINT8 pObjIdx)
{
	UINT32 FoundPendingObj = FALSE;
	UINT8 PendingIndex;
	...
	while(MAX_CONCURRENT_ACTIONS > PendingIndex)
	{
		...
		if ( (g_pCB->ObjPool[PendingIndex].ActionID == g_pCB->ObjPool[pObjIdx].ActionID) && 
			( (SL_MAX_SOCKETS == (g_pCB->ObjPool[PendingIndex].AdditionalData & BSD_SOCKET_ID_MASK)) || 
			((SL_MAX_SOCKETS > (g_pCB->ObjPool[pObjIdx].AdditionalData & BSD_SOCKET_ID_MASK)) && ( (g_pCB->ObjPool[PendingIndex].AdditionalData & BSD_SOCKET_ID_MASK) == (g_pCB->ObjPool[pObjIdx].AdditionalData & BSD_SOCKET_ID_MASK) ))) )
		{
	...
	if (!FoundPendingObj)
	{	
		if (SL_MAX_SOCKETS > (g_pCB->ObjPool[pObjIdx].AdditionalData & BSD_SOCKET_ID_MASK))
		{
			//unset socketID 
			g_pCB->ActiveActionsBitmap &= ~(1<<(g_pCB->ObjPool[pObjIdx].AdditionalData & BSD_SOCKET_ID_MASK));
		}
		else
		{
			//unset actionID 
			g_pCB->ActiveActionsBitmap &= ~(1<<g_pCB->ObjPool[pObjIdx].ActionID);
		}	
	}

	//delete old data
	g_pCB->ObjPool[pObjIdx].pRespArgs = NULL;
	g_pCB->ObjPool[pObjIdx].ActionID = 0;
	g_pCB->ObjPool[pObjIdx].AdditionalData = SL_MAX_SOCKETS;

	//remove from active list
	_SlRemoveFromList(&g_pCB->ActivePoolIdx, pObjIdx);
	//move to free list
	g_pCB->ObjPool[pObjIdx].NextIndex = g_pCB->FreePoolIdx;
	g_pCB->FreePoolIdx = pObjIdx;
...
void _SlRemoveFromList(UINT8 *ListIndex, UINT8 ItemIndex)
{
	...
	//need to remove the first item in the list and therfore update the global which holds this index
